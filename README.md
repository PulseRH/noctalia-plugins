...
- `media-mini`'s progress bar is text (block characters), not an actual background fill — bar widgets have no rect/opacity primitive exposed to plugins, only `setText`/`setGlyph`/`setImage`/`setColor`/etc. Because the bar characters need a monospace font to line up, having "Show progress bar" enabled overrides the widget's own Font setting to monospace; turn the bar off in the widget's settings to use whatever font you've picked instead. Rounded cover art shells out to `magick`/`convert` (ImageMagick) to bake a circular alpha mask; if neither is installed, art is shown unrounded instead.
