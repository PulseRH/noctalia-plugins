...
- **media-mini** — Now-playing via playerctl/MPRIS, preferring whichever player is actually Playing over a paused/stale one (e.g. a YouTube tab that navigated away, leaving dead MPRIS metadata behind), with rounded cover art (downloaded/cached for remote art, used directly for local files, falls back to a YouTube thumbnail when a browser's MPRIS doesn't expose real art; rounding needs ImageMagick, on by default and can be turned off in the widget's settings, which also expose compact/expand text width, cover size, and rounding amount). Scrolls as a marquee if still too long even expanded, and keeps polling while scrolling so a track change is never missed. Left-click opens the media panel, right-click toggles play/pause. Never touches font, so the widget's own Font setting always applies.
...
- `media-mini`'s rounded cover art shells out to `magick`/`convert` (ImageMagick) to bake a circular alpha mask; if neither is installed, art is shown unrounded instead.
